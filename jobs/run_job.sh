...
export IS_DEBUG_JOB
set +e
/usr/local/src/oswin-ci-2016/jobs/run_initialize.sh 2>&1
result_init=$?
...
if [ $result_init -eq 0 ]; then
    if [[ ! -z $RUN_TESTS ]] && [[ $RUN_TESTS == "no" ]]; then
        ...
        /usr/local/src/oswin-ci-2016/jobs/run_tests.sh 2>&1
        result_tempest=$?
        echo "$ZUUL_PROJECT;$ZUUL_BRANCH;$jen_date;$ZUUL_CHANGE;$ZUUL_PATCHSET;run;$result_tempest" >> /home/jenkins-slave/os-win-statistics.log
        echo "Tempest job finished with exit code $result_tempest"
    fi
fi

jen_date=$(date +%d/%m/%Y-%H:%M)
/usr/local/src/oswin-ci-2016/jobs/run_collect.sh 
result_collect=$?
