...
run_wsmancmd_with_retry $hyperv01 $WIN_USER $WIN_PASS 'powershell -executionpolicy remotesigned C:\OpenStack\oswin-ci\HyperV\scripts\collect_systemlogs.ps1'
...
run_wsmancmd_with_retry $hyperv02 $WIN_USER $WIN_PASS 'powershell -executionpolicy remotesigned C:\OpenStack\oswin-ci\HyperV\scripts\collect_systemlogs.ps1'
...
if [ "$IS_DEBUG_JOB" != "yes" ]
	then
		...
		scp -o "UserKnownHostsFile /dev/null" -o "StrictHostKeyChecking no" -i $LOGS_SSH_KEY "/home/jenkins-slave/logs/console-$ZUUL_UUID.log.gz" logs@logs.openstack.tld:/srv/logs/$logs_project/$ZUUL_CHANGE/$ZUUL_PATCHSET/console.log.gz
		...
		echo "Uploading temporary logs"
        	scp -o "UserKnownHostsFile /dev/null" -o "StrictHostKeyChecking no" -i $LOGS_SSH_KEY "/home/jenkins-slave/logs/console-$ZUUL_UUID.log.gz" logs@logs.openstack.tld:/srv/logs/debug/$logs_project/$ZUUL_CHANGE/$ZUUL_PATCHSET/$TIMESTAMP/console.log.gz
		...
		rm -fv aggregate-$VMID.tar.gz
                ...
                echo "Removine temporary console log"
                rm -fv /home/jenkins-slave/logs/console-$ZUUL_UUID.log.gz
fi
